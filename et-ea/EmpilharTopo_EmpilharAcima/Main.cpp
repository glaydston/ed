...
// empilharAcima: empilha o bloco A acima do bloco B
// removendo os blocos que estiverem sobre B e A
void empilharAcima(int a, int b, int retornoA, int retornoB, tPilha *p){
	tPilha aux;
	inicializa(&aux);
	int valorA, valorB, valorAux;
	if(acessaTopo(&valorA, (p + retornoA)) && valorA == a){
	...
	printf("\n\t\tEMPILHAR %d ACIMA %d", a, b);
}

// empilharTopo: empilha o bloco A e todos 
// que estiverem sobre A no topo do bloco B
void empilharTopo(int a, int b, int retornoA, int retornoB, tPilha *p){
	tPilha aux;
	inicializa(&aux);
	int valorA, valorB, valorAux;
		//verifica o valor do topo do bloco A
	...
	printf("\n\t\tEMPILHAR %d TOPO %d", a, b);
}

int findMajor(tCelula *aux){
	int maior = 0;
	while(aux != NULL){
		aux = aux->prox;
		maior++;
...
// encontraMaior: encontra a pilha que possui
// o maior numero de blocos e verifica qual é o
// maior bloco retornando-o para sua posição
void encontraMaior(tPilha *p){
	int maior, posicao;
	for(int i = 0; i < sizeof(p); i++){
		maior = findMajor((p + i)->topo->prox);
		for(int j = i + 1; j < sizeof(p); j++){
			if(maior <= findMajor((p + j)->topo->prox)){
				posicao = j;				
				printf("\n\t\tIF > Posicao: %d ", posicao);
			} else{
				posicao = i;
				printf("\n\t\tELSE > Posicao: %d ", posicao);
			}
		}	
	}
}
	...
	case 'E':		
			if(ch[1] == 'M')
				...
				if(a == b || retornoA == retornoB){
					...
					//empilhar acima
					if(ch[1] == 'A')
						empilharAcima(a, b, retornoA, retornoB, p);				
					//empilhar topo
					if(ch[1] == 'T')						
						empilharTopo(a, b, retornoA, retornoB, p);
					//encontra maior
				}
			}
		break;
